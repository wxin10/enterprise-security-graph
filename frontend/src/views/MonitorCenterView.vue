...
function buildTopologyChartOption() {
  ...
  return {
    ...
    series: [{ type: "graph", layout: "force", roam: true, draggable: true, focusNodeAdjacency: true, edgeSymbol: ["none", "arrow"], edgeSymbolSize: [6, 10], labelLayout: { hideOverlap: true }, force: { repulsion: 420, gravity: 0.06, edgeLength: [110, 200], friction: 0.08, layoutAnimation: true }, scaleLimit: { min: 0.45, max: 2.4 }, categories: categories.map((item) => ({ name: item.label })), data: chartNodes, links: chartLinks, lineStyle: { opacity: 0.82 }, emphasis: { focus: "adjacency", scale: true, lineStyle: { width: 4 } }, blur: { itemStyle: { opacity: 0.12 }, lineStyle: { opacity: 0.05 }, label: { opacity: 0.2 } } }]
  };
}
function bindTopologyChartEvents() {
  ...
  topologyChartInstance.on("finished", captureTopologyNodePositions);
...
function renderTopologyChart() { cleanupTopologyNodePositionState(); syncSelectedTopologyNode(); if (!topologyChartRef.value || topologyData.nodes.length === 0) { if (topologyChartInstance) topologyChartInstance.clear(); return; } ensureTopologyChart(); topologyChartInstance.setOption(buildTopologyChartOption(), true); topologyChartInstance.resize(); }
