...
## 13. BlockAction 与联动审批流模型设计

本系统中，`BlockAction` 并非孤立的动作记录，而是与图模型和底层审批链路深度绑定的核心实体。这一设计体现了自动化阻断与人工治理的结合：

1. **与安全威胁的闭环追溯**：
   `BlockAction` 通过 `DISPOSES` 关系反向指向它所处置的 `Alert` 节点。不仅让运维人员通过封禁日志追查根因成为可能，还能快速关联到触发告警的初始 `Session` 与 `Event`。
2. **与实体目标的网络映射**：
   `BlockAction` 通过 `TARGETS_IP` 关系直指恶意特征的载体 `IP` 节点。配合后续的自动化防火墙策略，可以从图谱直观查询出当前针对某一内网服务存活了几类阻断请求。
3. **审批流状态挂载**：
   在图谱中，`BlockAction` 的 `status` 属性（如 `PENDING_APPROVAL`, `BLOCKED`, `UNBLOCKED`）承担了审批流的流转功能。结合后端封禁接口的能力，系统的安全审计与工单审批实现了向“图元统一”转变。在管理控制台中的分析图表可以直接透出“哪些严重威胁已经生效封禁，哪些仍在等待人工审批阶段”，提升了安全态势的可量化性。
