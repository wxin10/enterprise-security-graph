...
class MonitorService:
    ...
    def _normalize_batch_payload(self, status_file_path: Path, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "unified_event_json_file": str(payload.get("unified_event_json_file", "") or ""),
            "unified_event_csv_file": str(payload.get("unified_event_csv_file", "") or ""),
            "unified_event_count": int(payload.get("unified_event_count", 0) or 0),
            "detected_attack_types": payload.get("detected_attack_types", []) or [],
            "blockable_event_count": int(payload.get("blockable_event_count", 0) or 0),
            "attack_behavior_json_file": str(payload.get("attack_behavior_json_file", "") or ""),
            "attack_behavior_csv_file": str(payload.get("attack_behavior_csv_file", "") or ""),
            "behavior_count": int(payload.get("behavior_count", 0) or 0),
            "behavior_types": payload.get("behavior_types", []) or [],
            "blockable_behavior_count": int(payload.get("blockable_behavior_count", 0) or 0),
            "high_risk_behavior_count": int(payload.get("high_risk_behavior_count", 0) or 0),
            "top_behavior_type": str(payload.get("top_behavior_type", "") or ""),
            "behavior_driven_enabled": bool(payload.get("behavior_driven_enabled", False)),
            "behavior_driven_used": bool(payload.get("behavior_driven_used", False)),
            "alert_count_from_behaviors": int(payload.get("alert_count_from_behaviors", 0) or 0),
            "block_candidate_count_from_behaviors": int(payload.get("block_candidate_count_from_behaviors", 0) or 0),
            "blocked_behavior_count": int(payload.get("blocked_behavior_count", 0) or 0),
            "top_blockable_behavior_type": str(payload.get("top_blockable_behavior_type", "") or ""),
            "auto_block_enabled": bool(payload.get("auto_block_enabled", False)),
            "auto_block_attempted": int(payload.get("auto_block_attempted", 0) or 0),
            "auto_block_success_count": int(payload.get("auto_block_success_count", 0) or 0),
            "auto_block_failed_count": int(payload.get("auto_block_failed_count", 0) or 0),
            "auto_blocked_ips": payload.get("auto_blocked_ips", []) or [],
            "auto_block_behavior_types": payload.get("auto_block_behavior_types", []) or [],
            "enforcement_mode": str(payload.get("enforcement_mode", "") or ""),
            "enforcement_success_count": int(payload.get("enforcement_success_count", 0) or 0),
            "verification_success_count": int(payload.get("verification_success_count", 0) or 0),
            "truly_blocked_count": int(payload.get("truly_blocked_count", 0) or 0),
            "top_auto_block_behavior_type": str(payload.get("top_auto_block_behavior_type", "") or ""),
            "failed_step": str(payload.get("failed_step", "") or "").strip(),
        ...
        recent_records = [
            {
                ...
                "unified_event_count": payload["unified_event_count"],
                "detected_attack_types": payload["detected_attack_types"],
                "blockable_event_count": payload["blockable_event_count"],
                "behavior_count": payload["behavior_count"],
                "behavior_types": payload["behavior_types"],
                "blockable_behavior_count": payload["blockable_behavior_count"],
                "high_risk_behavior_count": payload["high_risk_behavior_count"],
                "top_behavior_type": payload["top_behavior_type"],
                "behavior_driven_enabled": payload["behavior_driven_enabled"],
                "behavior_driven_used": payload["behavior_driven_used"],
                "alert_count_from_behaviors": payload["alert_count_from_behaviors"],
                "block_candidate_count_from_behaviors": payload["block_candidate_count_from_behaviors"],
                "blocked_behavior_count": payload["blocked_behavior_count"],
                "top_blockable_behavior_type": payload["top_blockable_behavior_type"],
                "auto_block_enabled": payload["auto_block_enabled"],
                "auto_block_attempted": payload["auto_block_attempted"],
                "auto_block_success_count": payload["auto_block_success_count"],
                "auto_block_failed_count": payload["auto_block_failed_count"],
                "auto_blocked_ips": payload["auto_blocked_ips"],
                "auto_block_behavior_types": payload["auto_block_behavior_types"],
                "enforcement_mode": payload["enforcement_mode"],
                "enforcement_success_count": payload["enforcement_success_count"],
                "verification_success_count": payload["verification_success_count"],
                "truly_blocked_count": payload["truly_blocked_count"],
                "top_auto_block_behavior_type": payload["top_auto_block_behavior_type"],
        ...
        for lane_index, batch_item in enumerate(recent_batches):
            ...
            add_node(
                ...
                detail_lines=[
                    ...
                    f"统一事件数：{batch_item['unified_event_count']}",
                    f"可封禁事件数：{batch_item['blockable_event_count']}",
